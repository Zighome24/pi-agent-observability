...
- `input`: prompt/input tokens reported by the provider.
- `output`: completion/output tokens generated by the provider.
- `cache_read` / `cache_write`: provider cache tokens; useful for context pressure and pricing analysis, but not the same as normal input/output tokens.
...
curl -sS -H "Authorization: Bearer $OBS_AUTH_TOKEN" \
  "$OBS_URL/usage/timeseries?from=2026-07-01T00:00:00Z&bucket=day&group_by=model"
